...
const INSIGHTS = [
  {
    ...
    relation: 'wired funds to',
    object: 'Advent Holdings',
    vacuity: 0.62,
    routing: 'escalate_now' as const,
  },
  {
    id: '2',
    subject: 'Advent Holdings',
    relation: 'shares address with',
    object: 'Meridian Supply LLC',
    vacuity: 0.58,
    routing: 'flag_for_review' as const,
  },
  {
    id: '3',
    subject: 'INV-4471',
    relation: 'invoiced by',
    object: 'Meridian Supply LLC',
    vacuity: 0.08,
    routing: 'auto_file' as const,
  },
  {
    id: '4',
    subject: 'Wilmington, DE',
    relation: 'registered address of',
    object: 'Advent Holdings',
    vacuity: 0.05,
    routing: 'auto_file' as const,
  },
...
function buildSpans(text: string) {
  const found = ENTITY_LABELS.map((entity) => {
    ...
    <div className="min-h-screen bg-ink-900">
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-10 px-6 py-20">
        ...
        <div className="inline-flex self-start overflow-hidden rounded-input border border-ctl-border bg-ctl-alt">
          {STEPS.map((s) => (
            <button
              ...
              onClick={() => setStep(s)}
              aria-pressed={step === s}
              className={`px-5 py-2.5 text-body-sm font-medium transition-colors duration-quick ease-out ${step === s ? 'bg-ctl text-ctl-fg' : 'text-ctl-alt-fg/80 hover:bg-ctl-alt-hover hover:text-ctl-alt-fg'}`}
            >
              {s}
            </button>
          ))}
        ...
        <div className="flex flex-wrap gap-3 border-t border-ink-500/40 pt-8">
          ...
          <Link
            to="/register"
            className="inline-flex items-center gap-2 rounded-input bg-ctl px-5 py-2.5 text-body-sm font-semibold text-ctl-fg transition-all duration-quick ease-out hover:-translate-y-0.5 hover:bg-verify hover:text-ctl-fg"
          >
...
function IngestStep() {
  ...
  for (const span of spans) {
    if (span.start > cursor)
      segments.push({ text: DOCUMENT_TEXT.slice(cursor, span.start), type: null });
    segments.push({ text: DOCUMENT_TEXT.slice(span.start, span.end), type: span.type });
    cursor = span.end;
  }
  if (cursor < DOCUMENT_TEXT.length)
    segments.push({ text: DOCUMENT_TEXT.slice(cursor), type: null });
        ...
        <p className="reader-body text-paper-text">
          {segments.map((seg, i) =>
            seg.type ? (
              <span
                key={i}
                className={`font-medium ${ENTITY_COLORS[seg.type] ?? 'text-paper-text'}`}
              >
                {seg.text}
                <span className="ml-1 font-mono text-micro opacity-60">{seg.type}</span>
              </span>
            ) : (
      ...
      <div className="panel flex items-center justify-center p-8">
        <svg viewBox="0 0 400 300" className="w-full max-w-md">
          ...
          <circle
            ...
            cy="60"
            r="24"
            className="fill-ink-700 stroke-ink-200"
            strokeWidth="1.5"
          />
          <text x="200" y="65" textAnchor="middle" className="fill-ink-50 text-[10px]">
            Meridian
          </text>
          ...
          <circle
            ...
            strokeWidth="1.5"
          />
          <text x="110" y="225" textAnchor="middle" className="fill-ink-50 text-[10px]">
            Advent
          </text>
          ...
          <rect
            ...
            strokeWidth="1.5"
          />
          <text x="290" y="225" textAnchor="middle" className="fill-ink-50 text-[10px]">
            INV-4471
          </text>
        </svg>
      </div>
      <p className="text-body-sm text-stamp-red">
        Cycle detected: Meridian ↔ Advent ↔ shared address
      </p>
    </div>
  );
}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        {escalated.map((insight) => (
          <div key={insight.id} className="panel flex items-center justify-between gap-4 px-4 py-3">
            <div className="text-body-sm text-ink-50">
              {insight.subject} <span className="text-ink-200">{insight.relation}</span>{' '}
              {insight.object}
            </div>
            <div className="flex items-center gap-3">
              <span className="font-mono text-body-sm text-ink-200">
                vac {insight.vacuity.toFixed(2)}
              </span>
      ...
      <button
        ...
        className="self-start rounded-input bg-verify px-4 py-2 text-body-sm font-medium text-ink-900 transition-colors duration-quick ease-out hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
      >
...
function ProveItStep() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-body-sm text-ink-200">
        Zero the key edge and re-run inference. If confidence actually changes, the explanation is
        causal.
      </p>
      ...
      <div className="panel flex flex-col gap-4 p-6">
        ...
        <div className="flex items-baseline gap-2">
          <span className="font-mono text-h1 text-verify">
            <CountUp
      ...
      {ran && (
        <PaperSurface className="max-w-2xl">
          <p className="reader-body text-paper-text">
            ...
            <span className="border-b-2 border-stamp-red bg-stamp-red/20">Advent Holdings</span> on
            behalf of Meridian Supply LLC, dated September 14th.
          </p>
          <p className="mt-2 font-mono text-body-sm text-ink-200">
