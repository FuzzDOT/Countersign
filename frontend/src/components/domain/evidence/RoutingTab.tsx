  ...
  if (query.isError && !query.data) {
    return (
      <ErrorState
        ...
        title="The routing results did not load"
      />
    );
  }
  if (!query.data) {
...
const perClassColumns: Column<PerClassMetrics>[] = [
  { key: 'bucket', header: 'Bucket', render: (r) => <RoutingBadge bucket={r.bucket} /> },
  {
    ...
    header: 'Precision',
    align: 'right',
    render: (r) => formatScore(r.precision),
  },
    ...
    <section aria-labelledby="routing-matrix" className="flex flex-col gap-4">
      ...
      <div className="grid gap-8 xl:grid-cols-[auto_1fr]">
        ...
        <div className="min-w-0 self-start">
          <DataTable
            caption="Precision, recall and F1 for each routing bucket"
    ...
    <div className="overflow-x-auto">
      <table className="border-separate border-spacing-1 text-body-sm">
        <caption className="sr-only">
          Confusion matrix. Rows are the ground truth bucket, columns are the predicted bucket.
        </caption>
        <thead>
          <tr>
            <td />
            ...
            <th
              ...
              className="pb-1 text-center font-medium text-ink-50"
            >
              Predicted
            </th>
            <td />
          ...
          <tr>
            ...
            {labels.map((label) => (
              <th
                key={label}
                scope="col"
                className="w-24 px-1 pb-1 text-center font-medium text-ink-200"
              >
                {name(label)}
              </th>
            ))}
  ...
  return (
    <section aria-labelledby="routing-cascade" className="flex flex-col gap-4">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <BarPanel
          title="Accuracy"
          label="Bar chart comparing routing accuracy of classical only, the cascade, and Nemotron on everything."
          data={accuracy}
        ...
        />
        <BarPanel
          title="Language model calls"
          label="Bar chart comparing the number of Nemotron calls made by the cascade and by running Nemotron on everything."
          data={calls}
...
function BarPanel({
  ...
  return (
    <div className="panel p-4">
      ...
      <div
        role="img"
        aria-label={`${label} ${data.map((d) => `${d.name}: ${format(d.value)}`).join('. ')}.`}
      >
          ...
          <BarChart data={data} margin={{ top: 24, right: 8, bottom: 8, left: 0 }}>
            ...
            <Bar dataKey="value" isAnimationActive={false} maxBarSize={72}>
              {data.map((d) => (
                <Cell
                  key={d.name}
                  fill={d.emphasis ? 'var(--ink-050)' : 'var(--ink-500)'}
                  stroke="var(--ink-200)"
                />
              ))}
              <LabelList
...
function FailureCases({
  ...
  failures: readonly DocumentedFailure[];
  onOpenInsight: (id: string) => void;
}) {
  return (
    <section aria-labelledby="routing-failures" className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 max-w-prose text-body text-ink-200">
          Cases the system routed differently from the labelled answer, with our own explanation of
          why.
        </p>
      </div>
      {failures.length === 0 ? (
        ...
        failures.map((failure) => (
          <FailureCaseCard key={failure.case_id} failure={failure} onOpenInsight={onOpenInsight} />
        ))
      )}
    </section>
  );
...
function FailureCaseCard({
  failure,
  onOpenInsight,
}: {
  failure: DocumentedFailure;
  onOpenInsight: (id: string) => void;
}) {
  ...
  return (
    <article className="flex flex-col gap-4 border-t border-ink-500/40 pt-6">
      <PaperSurface className="px-6 py-5">
        <p className="max-w-reader whitespace-pre-wrap break-words text-reader-body">
          {failure.sentence_text}
        </p>
      </PaperSurface>
    ...
    <div className="max-w-md">
      ...
      {long ? (
        <button
          ...
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          className="mt-1 text-body-sm text-ink-200 underline underline-offset-4 hover:text-ink-50"
        >
...
function NemotronAuditTable({ onOpenInsight }: { onOpenInsight: (id: string) => void }) {
  ...
  const columns: Column<NemotronRunOut>[] = [
    ...
    },
    {
      key: 'decision',
      header: 'Decision',
      sortValue: (r) => ROUTING_META[r.decision].rank,
      render: (r) => <RoutingBadge bucket={r.decision} />,
    },
    ...
    },
    {
      ...
      render: (r) =>
        r.input_tokens == null && r.output_tokens == null
          ? 'n/a'
          : `${r.input_tokens ?? '?'} and ${r.output_tokens ?? '?'}`,
    },
    {
      key: 'sha',
      header: 'Prompt SHA',
    ...
    <section aria-labelledby="routing-audit" className="flex flex-col gap-3">
      <div>
        ...
        <p className="mt-1 max-w-prose text-body-sm text-ink-200">
          Every call Nemotron was asked to make: what it decided, why it said so, how long it took,
          and a digest of the exact prompt.
        </p>
      </div>
      {query.isError && rows.length === 0 ? (
        ...
        <>
          <DataTable
            caption="Nemotron calls"
            columns={columns}
            rows={rows}
            rowKey={(r) => r.id}
            initialSort={{ key: 'time', dir: 'desc' }}
            emptyText="Nemotron has not been called yet."
          />
          {query.hasNextPage ? (
            <Button
              variant="secondary"
